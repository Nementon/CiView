...
namespace GraphSharp.Controls
{
        ...
        where TGraph : class, IBidirectionalGraph<TVertex, TEdge>
    {
        ...
        {
            foreach (var vertex in _vertexControls.Keys.ToArray()) RemoveVertexControl(vertex);
            foreach (var edge in _edgeControls.Keys.ToArray()) RemoveEdgeControl(edge);
        ...
        protected void RecreateGraphElements(bool tryKeepControls)
        {
            ...
            else
            {
                ...
                //
                foreach (var vertex in Graph.Vertices) if (!_vertexControls.ContainsKey(vertex)) CreateVertexControl(vertex);

                //
                // Presenters for edges
                //
                foreach (var edge in Graph.Edges) if (!_edgeControls.ContainsKey(edge)) CreateEdgeControl(edge);
        ...
        private void DoNotificationLayout()
        {
            ...
            _worker.RunWorkerCompleted += (s, e) =>
            {
                ...
                OnMutation();
                ContinueLayout();
                //if (HighlightAlgorithm != null) HighlightAlgorithm.ResetHighlight();
            };
        ...
        {
            if (!_vertexControls.ContainsKey(vertex)) CreateVertexControl(vertex);
        ...
        {
            if (!_edgeControls.ContainsKey(edge)) CreateEdgeControl(edge);
        ...
        protected virtual void CreateEdgeControl(TEdge edge)
        {
            ...
            if (ActualLayoutMode == GraphSharp.Algorithms.Layout.LayoutMode.Simple) Children.Insert(0, edgeControl);
        ...
        {
            var presenter = _vertexControls[vertex];
            RemoveFE(presenter);
            _vertexControls.Remove(vertex);
        }

        private void RemoveFE(FrameworkElement presenter)
        {
            var root = System.Windows.Media.VisualTreeHelper.GetChild(presenter, 0) as FrameworkElement;
            var grp = (from g in VisualStateManager.GetVisualStateGroups(root).OfType<VisualStateGroup>()
                       where g.Name == "CustomStates"
                       select g).FirstOrDefault();
            if (grp != null)
            {
                var storyboard = (from t in grp.Transitions.OfType<VisualTransition>()
                                  where t.From == "AfterAdded" && t.To == "BeforeRemoved"
                                  select t.Storyboard).FirstOrDefault();
                if (storyboard != null)
                {
                ...
                // This code doesn't work, due to possible bug in VSM
                //EventHandler<VisualStateChangedEventArgs> onStateChanged = null;
                //onStateChanged = (s, e) =>
                //{
                //    if (e.NewState.Name == "BeforeRemoved")
                //    {
                //        Children.Remove(presenter);
                //        grp.CurrentStateChanged -= onStateChanged;
                //    }
                //};
                //grp.CurrentStateChanged += onStateChanged;
                VisualStateManager.GoToState(presenter, "BeforeRemoved", true);
            }
            else Children.Remove(presenter);
        }


        protected virtual void RemoveEdgeControl(TEdge edge)
        {
            var presenter = _edgeControls[edge];
            RemoveFE(presenter);
