...
namespace GraphSharp.Controls
{
        ...
        where TGraph : class, IBidirectionalGraph<TVertex, TEdge>
    {
        ...
        protected IDictionary<TVertex, Size> _sizes;
        protected BackgroundWorker _worker;

        protected IDictionary<TVertex, Size> VertexSizes
        {
            get
            {
                if (_sizes == null)
                {
                    InvalidateMeasure();
                    UpdateLayout();
                    _sizes = new Dictionary<TVertex, Size>();
                    foreach (var kvp in _vertexControls)
                    {
                        var size = kvp.Value.DesiredSize;
                        _sizes.Add(kvp.Key, new Size(
                                                 (double.IsNaN(size.Width) ? 0 : size.Width),
                                                 (double.IsNaN(size.Height) ? 0 : size.Height)));
                    }
                }
                return _sizes;
            }
        ...
        protected IDictionary<TEdge, IList<Point>> RouteEdges(IDictionary<TVertex, Point> positions, IDictionary<TVertex, Size> sizes)
        {
